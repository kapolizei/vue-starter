...
watch(selectedProjectId, async (id) => {
  ...
  loading.value = true
  try {
    const res = await fetch(`https://project-rainfall-20113304171.development.catalystserverless.eu/api/tasks/${id}`)
    const data = await res.json()
...
const handleSubmit = async () => {
  ...
  try {
    const res = await fetch(
        `https://project-rainfall-20113304171.development.catalystserverless.eu/api/time/${selectedProjectId.value}`,
        {
          method: 'POST',
